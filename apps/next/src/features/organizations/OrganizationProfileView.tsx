...
export default function OrganizationProfileView({ user, profile, operationalContext, permissions, readiness, organization }: OrganizationProfileViewProps) {
  const complete = readiness.organization.complete;
  const missingFields = readiness.organization.missing_fields;

  return (
    <PlatformShell user={user} profile={profile} activePath="/organizzazione" operationalContext={operationalContext} permissions={permissions}>
      <section className={styles.page} aria-labelledby="organization-profile-title">
        ...
        <section className={styles.card} aria-labelledby="organization-data-title">
          <div className={styles.header}>
            <div><p className={styles.label}>Dati legali e contatti</p><h2 id="organization-data-title">{organization.display_name ?? 'Organizzazione'}</h2></div>
            <span className={styles.status} data-complete={complete}>{complete ? 'Completa' : `${missingFields.length} mancanti`}</span>
          </div>
          {!complete ? (
            <div className={styles.notice}>
              <p>Per questa organizzazione mancano:</p>
              <ul>{missingFields.map((field) => <li key={field}>{getReadinessLabel(field, organizationProfileFieldLabels)}</li>)}</ul>
            </div>
          ) : null}
